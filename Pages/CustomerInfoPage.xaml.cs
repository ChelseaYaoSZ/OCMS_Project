...
namespace OCMS
{
    ...
    public partial class CustomerInfoPage : Window
    {
        ...
        {
            string query = @"SELECT a.address, a.city, a.postal_code, p.person_id, a.address_id, c.customer_id,
        ...
        {
            string query = @"SELECT p.first_name, p.last_name, p.birth_date, p.phone, p.email, a.address,  c.prescription, c.customer_id 
                            FROM optic.customer c
                            Left JOIN optic.person p on p.person_id = c.person_id 
                            Left JOIN optic.address a on a.address_id = c.address_id 
